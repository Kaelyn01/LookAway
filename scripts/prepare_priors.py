...
teacher view (generated by build_trainset.py). Per-token log-probabilities are
...
Output: {data_dir}/token_priors_frozen.json  (keys: rate / tmpl / meta)
"""
...
def parse_args():
    ...
    ap.add_argument("--num-shards", type=int, default=1,
                    help="Split the dump across N processes (row i goes to shard i%%N); "
                         "shard outputs are merged later with --merge-shards")
    ap.add_argument("--shard", type=int, default=0, help="This process's shard index in [0, --num-shards)")
    ap.add_argument("--merge-shards", type=int, default=0,
                    help=">0: merge N shard outputs into teacher_posneg_token_scores.jsonl (no GPU needed), "
                         "then build the priors")
    return ap.parse_args()


def shard_row_indices(total: int, shard: int, num_shards: int) -> list[int]:
    """Row indices owned by one shard (round-robin); validates the shard spec."""
    if num_shards < 1:
...
def merge_dump_records(records: list[dict]) -> list[dict]:
    """Merge shard records into one dump in the original row order (by idx)."""
    ordered = sorted(records, key=lambda rec: rec["idx"])
    for n, rec in enumerate(ordered):
        rec["n"] = n
    return ordered
...
def run_dump(args, tok, proc, model):
    ...
    out_path = os.path.join(
        args.data_dir,
        f"teacher_posneg_token_scores.jsonl.part{args.shard:03d}" if sharded else "teacher_posneg_token_scores.jsonl",
    )
    with open(os.path.join(args.data_dir, "trainset_clean.jsonl"), encoding="utf-8") as f:
        all_rows = [json.loads(line) for line in f]

    results_path = os.path.join(args.data_dir, "dataset_generation_manifest.json")
    if not os.path.exists(results_path):
        raise FileNotFoundError("dataset_generation_manifest.json is required; run scripts/build_trainset.py first")
    ...
    if actual_ids != expected_ids or not all(
        ...
        raise ValueError(
            "dataset_generation_manifest.json is incomplete or was not generated by the aligned v2 pipeline"
        )
    rows = all_rows[: args.limit] if args.limit > 0 else all_rows
    total = len(rows)
    targets = shard_row_indices(total, args.shard, args.num_shards)
    print(f"Shard {args.shard}/{args.num_shards}: {len(targets)} of {total} rows")
...
def run_priors(args, dump_path, tok):
    ...
    meta = {
        ...
        "generation_results_sha256": file_sha256(os.path.join(args.data_dir, "dataset_generation_manifest.json")),
        "train_parquet_sha256": file_sha256(os.path.join(args.data_dir, "trainset_clean.parquet")),
        "alpha": ALPHA,
        "base": BASE,
    ...
    out = os.path.join(args.data_dir, "token_priors_frozen.json")
...
def mark_freq_table_stale(data_dir: str) -> None:
    """The token dump changed: any existing frequency table is now stale."""
    freq_path = os.path.join(data_dir, "token_freq_counts.json")
    if os.path.exists(freq_path):
        os.replace(freq_path, freq_path + ".stale")
        print(f"Marked stale (token dump changed): {freq_path} -> {freq_path}.stale")


def merge_shards(args) -> str:
    """Merge shard dump outputs into teacher_posneg_token_scores.jsonl (no GPU needed)."""
    records = []
    for shard in range(args.merge_shards):
        part = os.path.join(args.data_dir, f"teacher_posneg_token_scores.jsonl.part{shard:03d}")
        if not os.path.exists(part):
            raise SystemExit(f"Missing shard output {part}")
        with open(part, encoding="utf-8") as stream:
            records.extend(json.loads(line) for line in stream if line.strip())
    merged = merge_dump_records(records)
    full = os.path.join(args.data_dir, "teacher_posneg_token_scores.jsonl")
    tmp = full + ".merging"
    ...
    os.replace(tmp, full)
    for shard in range(args.merge_shards):
        os.remove(os.path.join(args.data_dir, f"teacher_posneg_token_scores.jsonl.part{shard:03d}"))
    print(f"Merged {len(merged)} records from {args.merge_shards} shards -> {full}")
    mark_freq_table_stale(args.data_dir)
    return full
...
def main():
    ...
    if args.num_shards > 1:
        print(
            f"Shard {args.shard} dump complete; rerun with "
            f"--merge-shards {args.num_shards} to merge and build the priors."
        )
        return
    mark_freq_table_stale(args.data_dir)
    run_priors(args, dump_path, tok)
