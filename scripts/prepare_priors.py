...
def parse_args():
    ...
    ap.add_argument("--merge-shards", type=int, default=0,
                    help=">0: merge N shard outputs into token_scores_full.jsonl (no GPU needed), "
...
def run_dump(args, tok, proc, model):
    ...
    out_path = os.path.join(
        args.data_dir,
        f"token_scores_full.jsonl.part{args.shard:03d}" if sharded else "token_scores_full.jsonl",
    )
...
def mark_freq_table_stale(data_dir: str) -> None:
    ...
    freq_path = os.path.join(data_dir, "token_freq.json")
...
def merge_shards(args) -> str:
    """Merge shard dump outputs into token_scores_full.jsonl (no GPU needed)."""
    records = []
    for shard in range(args.merge_shards):
        part = os.path.join(args.data_dir, f"token_scores_full.jsonl.part{shard:03d}")
    ...
    full = os.path.join(args.data_dir, "token_scores_full.jsonl")
    tmp = full + ".merging"
    ...
    os.replace(tmp, full)
    for shard in range(args.merge_shards):
        os.remove(os.path.join(args.data_dir, f"token_scores_full.jsonl.part{shard:03d}"))
