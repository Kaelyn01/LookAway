...
class vLLMAsyncRollout(BaseRollout):
    ...
    def _init_worker(self, all_kwargs: list[dict[str, Any]]):
        ...
        distributed_executor_backend = getattr(parallel_config, "distributed_executor_backend", None)
        if (
            is_version_ge(pkg="vllm", minver="0.18.0")
            and isinstance(distributed_executor_backend, type)
            and distributed_executor_backend.__name__ == "ExternalZeroMQDistributedExecutor"
        ):
            # vLLM 0.18 treats custom executor classes as in-process executors
            # during worker device init. SDPO launches one Ray actor per GPU, so
            # mark the worker-side config as external to avoid requiring each
            # actor to see all TP devices.
    ...
    async def _execute_method(self, method: str | bytes, *args, **kwargs):
        ...
        elif isinstance(method, str) and hasattr(self.inference_engine, method):
            return getattr(self.inference_engine, method)(*args, **kwargs)
        elif isinstance(method, str) and hasattr(getattr(self.inference_engine, "worker", None), method):
            return getattr(self.inference_engine.worker, method)(*args, **kwargs)
        elif hasattr(self.inference_engine, "execute_method"):
            return self.inference_engine.execute_method(method, *args, **kwargs)
        else:
            raise AttributeError(f"{type(self.inference_engine).__name__} does not support method {method!r}")
