...
class vLLMHttpServer:
    ...
    def __init__(
        ...
        _hf_max_len = get_max_position_embeddings(self.model_config.hf_config)
        # vllm>=0.26 strictly reserves KV cache by max_model_len; never enlarge an explicitly
        # configured value (Qwen3.5 config declares 262144, which would demand 262GB KV).
        if self.config.max_model_len is None or self.config.max_model_len <= 0:
            self.config.max_model_len = _hf_max_len
        else:
            self.config.max_model_len = min(self.config.max_model_len, _hf_max_len)
    ...
    async def generate(
        ...
        # Validation can request a longer response length while training keeps the default.
        configured_max_new_tokens = self.config.response_length
        if "max_tokens" in sampling_params:
            max_tokens = sampling_params.pop("max_tokens")
            configured_max_new_tokens = max(configured_max_new_tokens, max_tokens)
        ...
        # Clamp max_tokens to the valid range [0, min(requested response length, remaining context)].
        max_tokens = max(0, min(max_tokens, configured_max_new_tokens, max_possible_tokens))
