...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        vd_neg_keys = {
            ...
            "teacher_neg_response_start_idx",
            "vd_neg_view_mask",
        }
        if vd_neg_keys.issubset(set(data.batch.keys())):
            select_keys.extend(sorted(vd_neg_keys))
        ...
        if self._has_non_empty_multi_modal_inputs(data.non_tensor_batch.get("teacher_neg_multi_modal_inputs")):
            non_tensor_select_keys.append("teacher_neg_multi_modal_inputs")
                ...
                for micro_batch in micro_batches:
                    ...
                    if self_distillation_enabled:
                        ...
                        if vd_gamma and vd_gamma > 0:
                            if "teacher_neg_input_ids" not in model_inputs:
                                raise KeyError("vd_gamma > 0 requires teacher_neg_* tensors in the batch.")
                            teacher_neg_inputs = {
                                "responses": model_inputs["responses"],
                                ...
                                "response_start_idx": model_inputs["teacher_neg_response_start_idx"],
                            }
                            ...
                            if self_distillation_cfg.get("vd_dual_signal", False):
                                ...
                                if has_valid:
                                    ...
                                    tau_dd = abs_dd.new_tensor(0.25)
                                s_dd = abs_dd / (abs_dd + tau_dd)
                                eps = (log_prob - teacher_log_prob).detach().abs()
                                if has_valid:
                                    tau_e = eps[valid_pos].quantile(0.9).clamp(min=0.25)
                                else:
                                    tau_e = eps.new_tensor(0.25)
                                s_err = eps / (eps + tau_e)
                                if self_distillation_cfg.get("vd_base_signal", "dual") == "dd":
                                    # dd-only base weight: the eps axis moves out of the multiplicative
                                    # base and into the eligibility gates (JSD ammo gate).
                                    bonus = s_dd
                                else:
                                    bonus = s_dd * s_err
                            else:
                                ...
                                if self_distillation_cfg.get("vd_jsd_gate", False):
                                    ...
                                    with torch.no_grad():
                                        ...
                                        if self_distillation_cfg.distillation_add_tail:
                                            s_topk = self._add_tail_bucket(s_topk)
                                            t_topk = self._add_tail_bucket(t_topk)
                                        else:
