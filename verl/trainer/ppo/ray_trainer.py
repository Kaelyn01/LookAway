...
class RayPPOTrainer:
    ...
    def _build_neg_teacher_inputs(
        ...
        teacher under the negative view. Only called when vd_gamma > 0.
        """
        neg_image_key = self_distillation_cfg.get("teacher_neg_image_key", None)
        if neg_image_key is None:
            raise ValueError("vd_gamma > 0 requires self_distillation.teacher_neg_image_key.")
        if neg_image_key not in batch.non_tensor_batch:
            raise KeyError(f"Teacher neg image key `{neg_image_key}` not found in batch.non_tensor_batch")
        ...
        for i in range(batch_size):
            ...
            neg_images = batch.non_tensor_batch[neg_image_key][i]
            if isinstance(neg_images, np.ndarray):
                neg_images = neg_images.tolist()
            elif neg_images is None:
                neg_images = []
            else:
        ...
        tensors = {
            ...
            "teacher_neg_attention_mask": teacher_attention_mask,
            "teacher_neg_position_ids": teacher_position_ids,
            "teacher_neg_response_start_idx": torch.stack(teacher_response_start_idx_list).to(device),
            "vd_neg_view_mask": torch.tensor(teacher_present_mask_list, dtype=torch.float32, device=device),
        }
        return tensors, {"teacher_neg_multi_modal_inputs": teacher_multi_modal_inputs_list}, metrics
    ...
    def _maybe_build_self_distillation_batch(
        ...
        if use_teacher_always_on_inputs:
            ...
            vd_gamma = self_distillation_cfg.get("vd_gamma", 0.0)
            if vd_gamma and vd_gamma > 0:
                neg_tensors, neg_non_tensors, neg_metrics = self._build_neg_teacher_inputs(batch, self_distillation_cfg, device)
                metrics.update(neg_metrics)
            else:
                neg_tensors, neg_non_tensors = {}, {}
            ...
            return DataProto.from_dict(
                ...
                non_tensors={"teacher_multi_modal_inputs": teacher_multi_modal_inputs_list, **neg_non_tensors},
            ), metrics
    ...
    def _get_gen_batch(self, batch: DataProto) -> DataProto:
        ...
        teacher_neg_image_key = self.config.actor_rollout_ref.actor.get("self_distillation", {}).get("teacher_neg_image_key", None)
        if teacher_neg_image_key and teacher_neg_image_key in batch.non_tensor_batch:
            reward_model_keys.add(teacher_neg_image_key)
