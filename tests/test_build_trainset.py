...
class PrepareDataTests(unittest.TestCase):
    def test_split_archive_extraction_rejects_path_traversal(self):
        ...
        module = load_script("build_trainset_archive", "scripts/build_trainset.py", fake_modules)
    ...
    def test_dataset_paths_cannot_escape_data_directory(self):
        ...
        module = load_script("build_trainset_paths", "scripts/build_trainset.py", fake_modules)
        with tempfile.TemporaryDirectory() as tmp:
            self.assertEqual(
    ...
    def test_default_upstream_revision_is_pinned(self):
        ...
        module = load_script("build_trainset_revision", "scripts/build_trainset.py", fake_modules)
    ...
    def test_generation_uses_parent_row_count(self):
        ...
        module = load_script("build_trainset", "scripts/build_trainset.py", fake_modules)
        module.Pool = FakePool
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "trainset_clean.jsonl"
    ...
    def test_missing_negative_view_keeps_parquet_column(self):
        ...
        module = load_script("build_trainset_record", "scripts/build_trainset.py", fake_modules)
        item = {
            "problem": "question",
            "images": ["student.png"],
    ...
    def test_generation_failure_blocks_incomplete_dataset(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            old_train = Path(tmp) / "trainset_clean.parquet"
            old_train.write_bytes(b"old-train")
            old_priors = Path(tmp) / "token_priors_frozen.json"
            old_priors.write_text("{}", encoding="utf-8")
            (Path(tmp) / "trainset_clean.jsonl").write_text('{"id": 1}' + chr(10), encoding="utf-8")
    ...
    def test_successful_regeneration_invalidates_training_artifacts(self):
        ...
        module = load_script("build_trainset_atomic", "scripts/build_trainset.py", fake_modules)
        module.Pool = FakePool
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "trainset_clean.jsonl").write_text('{"id": 1}' + chr(10), encoding="utf-8")
            old_negative = root / "teacher_neg" / "old.png"
            old_negative.parent.mkdir()
            old_negative.write_bytes(b"old-negative")
            (root / "trainset_clean.parquet").write_bytes(b"old-train")
            (root / "token_priors_frozen.json").write_text("old-priors", encoding="utf-8")
            (root / "dataset_generation_manifest.json").write_text("old-results", encoding="utf-8")
            module.generate_negative_views(tmp, 1)
            self.assertFalse(old_negative.exists())
            self.assertEqual((root / "teacher_neg/000000.png").read_bytes(), b"negative")
            self.assertFalse((root / "trainset_clean.parquet").exists())
            self.assertFalse((root / "token_priors_frozen.json").exists())
            self.assertEqual((root / "trainset_clean.parquet.stale").read_bytes(), b"old-train")
            self.assertEqual((root / "token_priors_frozen.json.stale").read_text(encoding="utf-8"), "old-priors")
            stale = root / "dataset_generation_manifest.json.stale"
            self.assertEqual(stale.read_text(encoding="utf-8"), "old-results")
            self.assertNotEqual((root / "dataset_generation_manifest.json").read_text(encoding="utf-8"), "old-results")
    ...
    def test_missing_staged_image_blocks_publish(self):
        ...
        module = load_script("build_trainset_missing_file", "scripts/build_trainset.py", fake_modules)
    ...
    def test_parquet_conversion_rejects_incomplete_results(self):
        ...
        module = load_script("build_trainset_parquet", "scripts/build_trainset.py", fake_modules)
        with tempfile.TemporaryDirectory() as tmp:
            (Path(tmp) / "trainset_clean.jsonl").write_text('{"problem": "q"}' + chr(10), encoding="utf-8")
...
class PreparePriorsTests(unittest.TestCase):
    ...
    def test_prior_dump_requires_aligned_generation_results(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            args.data_dir = tmp
            (root / "trainset_clean.jsonl").write_text('{"problem": "q"}' + chr(10), encoding="utf-8")
            with self.assertRaisesRegex(FileNotFoundError, "dataset_generation_manifest.json is required"):
                self.module.run_dump(args, object(), object(), object())
            (root / "dataset_generation_manifest.json").write_text(
